...
#工程名 将XXX替换成自己的工程名
project_name=TeeMoTest

#scheme名 将XXX替换成自己的sheme名
scheme_name=TeeMoTest
...
if [ $number == 1 ];
    ...
    #验证并上传到App Store
    # 将-u 后面的XXX替换成自己的AppleID的账号，-p后面的XXX替换成自己的密码，第二个-p如果有双重验证则需要写app specific密码

    altoolPath="/Applications/Xcode.app/Contents/Applications/Application Loader.app/Contents/Frameworks/ITunesSoftwareService.framework/Versions/A/Support/altool"
    "$altoolPath" --validate-app -f ${exportIpaPath}/${scheme_name}.ipa -u XX [-p XX]
    "$altoolPath" --upload-app -f ${exportIpaPath}/${scheme_name}.ipa -u  XX -p XXX
else
